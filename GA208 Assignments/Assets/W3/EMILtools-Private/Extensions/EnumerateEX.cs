...
namespace EMILtools.Extensions
{
    public static class EnumerateEX
    {
        ...
        {
            if( ts == null || ts.Length <= 0) throw new InvalidOperationException("Trying to get a random element from an empty array");
        ...
        {
            if (ts == null || ts.Count <= 0) throw new InvalidOperationException("Trying to get a random element from an empty list");
        ...
        public static bool Contains(this GameObject[] list, GameObject target, out GameObject match)
        {
        ...
        public static List<MonoBehaviour> SetAllActive(this List<MonoBehaviour> list, bool val)
        {
            for(int i = 0; i < list.Count; i++) list[i].gameObject.SetActive(val);
            return list;
        }
        
        public static MonoBehaviour[] SetAllActive(this MonoBehaviour[] list, bool val)
        {
            for (int i = 0; i < list.Length; i++) list[i].gameObject.SetActive(val);
            return list;
        }
