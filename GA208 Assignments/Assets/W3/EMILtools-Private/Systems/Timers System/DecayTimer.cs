...
namespace EMILtools.Timers
{
    ...
    {
        [ShowInInspector] ClassRef<float> decayMult;

        [Button]
        public DecayTimer(float initialValue, float _decayMult) : base(initialValue)
         => decayMult = _decayMult;
        public DecayTimer(float initialValue, ClassRef<float> _decayMult) : base(initialValue)
            => decayMult = _decayMult;
        public DecayTimer(ClassRef<float> initialValue, ClassRef<float> _decayMult) : base(initialValue)
            => decayMult = _decayMult;
